...
load('/Volumes/JHPCE/dcs01/active/projects/GTeX/syang/look.version8/R_objects/sig.spear.tiss.rds')
...
dplyr::select(sig, Spear.Correlation, lowerCI, upperCI)
...
for(i in 1: nrow(w.perm)){
  if(w.perm$Sig[i] == 1){
    ...
    w.perm$lowerCI[i] = format(round(as.numeric(w.perm$lowerCI[i]), 3))
    w.perm$upperCI[i] = format(round(as.numeric(w.perm$upperCI)[i], 3))
    w.perm$CI[i] = paste0(w.perm$lowerCI[i], ';', w.perm$upperCI[i])

    w.perm$Spear.Correlation[i] = format(round(as.numeric(w.perm$Spear.Correlation[i]), 3))
    w.perm$mean[i] = format(round(as.numeric(w.perm$mean[i]), 3))
