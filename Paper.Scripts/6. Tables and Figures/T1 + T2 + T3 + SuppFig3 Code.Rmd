...
important$pval <-ifelse(important$pval > 0.001, format(round(important$pval, 3), nsmall = 3), formatC(important$pval, format = "e", digits = 2))
...
important$T.test.pval <-ifelse(important$T.test.pval > 0.001, format(round(important$T.test.pval, 3), nsmall = 3)
...
# formatting
means <- c(NA, NA, NA, means, NA)
upper <- c(NA, NA, NA, upper, NA)
lower <- c(NA, NA, NA, lower, NA)

# create table text
text <-cbind(c("36 MT-encoded genes in whole blood", NA, "Genes", mt$symbol, NA), c(NA, NA,"Effect estimate", formatC(mt$beta, format = "e", digits = 2), NA))

	# draw forestplot
pdf(paste0('~/Desktop/Nice.Screenshots/mtrna.forestplot.pdf'), width = 10, height = 8, onefile = F)
	forestplot(text, means, lower, upper, col=fpColors(box="royalblue",line="darkblue", summary="royalblue"), is.summary=c(TRUE, FALSE, TRUE, rep(FALSE, nrow(mt)+1), TRUE),
