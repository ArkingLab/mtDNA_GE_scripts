...
# Modified 11.15.2020 --> Use 1000 permutations
...
if (length(args)!=2) {
  stop("Two arguments must be supplied (fileID).", call.=FALSE)
 }

num<-args[1]
num <- as.numeric(num)

perm.num = as.numeric(args[2]) # 1:100 are already done for the other tissues. so just need 101:1000
startnum = perm.num * 100
endnum = startnum + 99
...
if (!file.exists('with.gene.rds')){print(paste0('with.gene.rds does not exist for ', char))} else{
	...
	save(all.perm.cutoff.forset, file = paste0('all.perm.cutoff.forset_', startnum, '.rds'))
}
